...
The examples contains two tasks, \#loopTask and \#cycleTask, with a \#loopTask
...
change configures the system so that it can execute its next task, and insures
that all previous steps were executed properly. Thus the internal logic of when
a task is complete remain internal to the workflow and no external logic is
required to satisfy task completion.

This example shows a very interesting relationship between the \#hasCondition
and \#dependsOn object properties in the ontology: Conditions are just special,
internal dependencies that are managed directly by the task instead of the
...
first executing the tasks on which it depends. Thus conditions can be thought of
as merely subgraphs of the larger workflow that sit between a task and its
dependencies.
...
\cite{sns-hfir-data-review}. This example is a good example of the conceptual
workflows mentioned in chapter \ref{ch:introduction}, \S \ref{workflows} because
it cannot be executed and represents high-level, idealized tasks.
...
The graph of this workflow when translated to the workflow ontology is shown in
figure \ref{neutron-workflow-graph}. One immediate observation is that
...
points, such as ``HPC, Modeling/Simulation, AI/ML'' because they approach the
...
This chapter presented the methodology and reasoning behind creating a workflows
...
examples in particular demonstrated the central thesis of this work that a
...
further demonstrated that common patterns can be described by the ontology
